...
#define MSG_BUFFER_SIZE  256                                                       // for MQTT message payload
...
float medianPressure, temperature;                              
...
boolean reconnect() {
  ...
  if (mqttClient.connect(DEVICE_NAME, MQTT_USER_NAME, MQTT_PASSWORD, LWT_TOPIC, 2, true, "Disconnected")) {
    ...
    sprintf(msg, "{\"valveState\": \"%d\", \"idlePublishInterval\": \"%d\", \"minPublishInterval\": \"%d\", \"sensorReadInterval\": \"%d\", \"pressureChange\": \"%.2f\"}", 
                   valveState, opParams.idlePublishInterval, opParams.minPublishInterval, opParams.sensorReadInterval, opParams.pressureChange);
...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  if(strstr(topic, "pressureChange"))                    // publish if pressure changes this amount
  {
    cmdValid = true;
    if(atof(msg) > .1)
    {
  ...
  if(strstr(topic, "readParams"))                         // reload params from file without reboot or file write
  {
    ...
    if(paramFileObj.readBytes((char *)&opParams, sizeof(opParams)) > 0 )
    {
      ...
      Serial.printf( "{\"valveState\": \"%d\", \"idlePublishInterval\": \"%d\", \"minPublishInterval\": \"%d\", \"sensorReadInterval\": \"%d\", \"pressureChange\": \"%.2f\"}\n", 
                       valveState, opParams.idlePublishInterval, opParams.minPublishInterval, opParams.sensorReadInterval, opParams.pressureChange);
    }
    else
  ...
  {
    cmdValid = true;
    sprintf(msg, "{\"commands\" : \"valveState, idlePublishInterval, minPublishInterval, sensorReadInterval, pressureChange, reportParams, defaultPParams, readParams, writeParams, deleteParams, reboot, help\"}");
...
void setup() {
  ...
  if (!myTZ.setCache(TIMEZONE_EEPROM_OFFSET))                        // using EEPROM just becuase it's built into ezTime
  ...
  if (LittleFS.exists(F(PARAMS_FILENAME)))                          // if file exists
  {
    ...
    if(paramFileObj.readBytes((char *)&opParams, sizeof(opParams)) > 0)
    {
      ...
      Serial.printf( "{\"idlePublishInterval\": \"%d\", \"minPublishInterval\": \"%d\", \"sensorReadInterval\": \"%d\", \"pressureChange\": \"%.2f\"}\n\n", 
                     opParams.idlePublishInterval, opParams.minPublishInterval, opParams.sensorReadInterval, opParams.pressureChange);
    }
    else
    {
  ...
  {                                                                 // fill it with default values
    Serial.println(F("No parameters file detected"));
    opParams.idlePublishInterval = DEFAULT_IDLE_PUBLISH_INTERVAL_MS;
    opParams.minPublishInterval = DEFAULT_MIN_PUBLISH_INTERVAL_MS;
    opParams.sensorReadInterval = DEFAULT_SENSOR_READ_INTERVAL_MS;
    opParams.pressureChange = (float)DEFAULT_PRESSURE_CHANGE_PSI;
...
void loop() {
  ...
  ArduinoOTA.handle();
  events();                                                           // process ezTime events if any
  ...
  if( (opParams.idlePublishInterval <= DEFAULT_SENSOR_READ_INTERVAL_MS) || (opParams.minPublishInterval <= DEFAULT_SENSOR_READ_INTERVAL_MS)
     || (opParams.sensorReadInterval <= 3)  || (opParams.pressureChange <= (float).1) )
  {
    Serial.printf( "{\"valveState\": \"%d\", \"idlePublishInterval\": \"%d\", \"minPublishInterval\": \"%d\", \"sensorReadInterval\": \"%d\", \"pressureChange\": \"%.2f\"}\n", 
                 valveState, opParams.idlePublishInterval, opParams.minPublishInterval, opParams.sensorReadInterval, opParams.pressureChange);
    opParams.idlePublishInterval = DEFAULT_IDLE_PUBLISH_INTERVAL_MS;
